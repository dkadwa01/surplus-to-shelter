...
export function RecipientProfileForm({ initial, creating, onSave, onCancel }: {
  ...
  return <form className="auth-form recipient-form" onSubmit={submit} noValidate>
    ...
    <LocationSearch title="Search recipient location" markerType="RECIPIENT" value={fields.address} latitude={fields.latitude} longitude={fields.longitude} onAddressChange={(address) => setFields((current) => ({ ...current, address, latitude: undefined, longitude: undefined, serviceRadiusKm: undefined }))} onSelect={(location) => setFields((current) => ({ ...current, address: location.address, serviceArea: location.area || current.serviceArea, latitude: location.latitude, longitude: location.longitude }))} />
