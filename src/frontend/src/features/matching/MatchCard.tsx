...
export function MatchCard({ match }: { match: MatchingResult }) {
  ...
  return <article className="match-card">
    <div className="match-card-head"><div><p className="eyebrow">{sourceLabel}</p><h2>{match.source.foodName}</h2></div><span className="match-score">{match.eligible ? "Suitable" : "Review"}</span></div>
    ...
    <div className="match-metrics"><span>{match.distanceKm === null ? "Distance unavailable" : `Approx. distance: ${match.distanceKm.toFixed(1)} km straight-line`}</span><span>{match.quantityCompatibility.replace(/_/g, " ").toLowerCase()}</span><span>Use by {new Date(match.source.expiresAt).toLocaleString()}</span></div>
