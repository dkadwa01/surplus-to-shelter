...
export function DonationDetailsPage() {
  ...
  const mapPoints: DashboardSummary["mapPoints"] = donation.latitude !== null && donation.longitude !== null ? [{ id: donation.id, type: "DONATION", label: donation.foodName, area: donation.pickupArea, latitude: donation.latitude, longitude: donation.longitude }] : [];
  return <article className="donation-detail"><Link to="/donations">Back to all donations</Link><div className="donation-card-top"><span className={`status-pill status-${donation.status.toLowerCase()}`}>{donation.status}</span><span>{donation.category.replace(/_/g, " ")}</span></div><h1>{donation.foodName}</h1><p className="intro">{donation.quantity} {donation.unit.toLowerCase()}</p><dl><dt>Pickup location</dt><dd>{donation.pickupAddress}, {donation.pickupArea}</dd><dt>Use by</dt><dd>{new Date(donation.expiresAt).toLocaleString()}</dd>{donation.preparedAt && <><dt>Prepared</dt><dd>{new Date(donation.preparedAt).toLocaleString()}</dd></>}{donation.description && <><dt>Description</dt><dd>{donation.description}</dd></>}<dt>Donor type</dt><dd>{donation.donorType?.replace(/_/g, " ") ?? "Donor"}</dd><dt>Delivery</dt><dd>Dispatch tracking is not available in this checkout.</dd></dl>
    <LocationMap points={mapPoints} title="Donation pickup location" />
    {donation.status === "AVAILABLE" && <div className="form-actions"><Link className="secondary-button" to={`/donations/${id}/edit`}>Edit</Link><Link className="secondary-button" to={`/matching/donations/${id}`}>Find recipients</Link><button className="danger-button" disabled={busy} onClick={() => void cancel()}>{busy ? "Cancelling..." : "Cancel donation"}</button></div>}{error && <p className="form-error" role="alert">{error}</p>}</article>;
}
