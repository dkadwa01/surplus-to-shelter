...
Group lifecycle: `OPEN -> TARGET_REACHED -> CLOSED`, or `OPEN -> CLOSED|CANCELLED|EXPIRED`; an open group may also expire. Reached/closed/cancelled/expired collections reject contribution changes. Displayed totals keep category/unit groups separate. Target progress uses current, unexpired contributions in the target category and unit; it safely converts between kilograms and grams, clamps progress to 100%, and never combines incompatible units. A community contribution below a configured individual-post threshold is accepted as a collective exception and flagged only in its owner's response. No threshold is preconfigured, so no arbitrary quantity is assumed. Expiration is processed when collections are read or changed.
...
Candidate lists contain only eligible pairs and are capped at 100 direct donations, 1,000 community contribution items, or 100 recipient profiles per request. Responses omit donor identity, street addresses, recipient contact details, and exact coordinates. Display distance is rounded to one decimal kilometer and is straight-line distance, not a route estimate. An unavailable/expired individual donation returns `409`; a community collection must be `TARGET_REACHED` or `CLOSED`.

## Dashboard

`GET /api/dashboard/summary` requires an authenticated session and returns role-scoped summary data used by the prototype dashboards. It reads current donations, community contributions, recipient profile/verification state, and on-demand matching results; it does not create a dashboard-specific database record. Donation quantities exclude cancelled listings, aggregate compatible quantities across categories, keep incompatible units separate, and normalize grams to kilograms. Matching counts are current suggestions only and are marked unavailable if source lists exceed the existing matching screens' limits. Delivery, assignment, meals-served, and recipient-served metrics are not available because those records and a conversion rule do not exist in this checkout. Map points contain coordinates already available to the user's existing role-scoped APIs; they contain no street addresses. Map distances are straight-line estimates.

`GET /api/locations/search?q=Jaipur` requires an authenticated session. It is called only after the user explicitly submits a location search. The backend caches results in memory and serializes upstream searches to at most one request per 1.1 seconds per backend process. The provider URL is configurable with `LOCATION_SEARCH_URL`.
